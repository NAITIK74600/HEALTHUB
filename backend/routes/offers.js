  ...
  try {
    ...
    await execute(
      `UPDATE offers SET title = ?, description = ?, image_url = ?, link = ?, badge = ?,
       is_active = ?, start_date = ?, end_date = ?, ord = ? WHERE id = ?`,
      [title, description || '', imageUrl || '', link || '', badge || '',
       isActive !== false ? 1 : 0, startDate || null, endDate || null, ord || 0, req.params.id]
    );
  ...
  try {
    ...
    const result = await execute(
      `INSERT INTO offers (title, description, image_url, link, badge, is_active, start_date, end_date)
       VALUES (?, ?, ?, ?, ?, 0, ?, ?)`,
      [`${o.title} (Copy)`, o.description, o.image_url, o.link, o.badge, o.start_date, o.end_date]
    );
