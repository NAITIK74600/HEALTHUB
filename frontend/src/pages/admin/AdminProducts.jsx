...
export default function AdminProducts() {
  ...
  const handleInlineUpload = async (e) => {
    ...
    try {
      ...
      const { data } = await updateProduct(uploadingFor, fd);
  ...
  const handleImageUrl = async () => {
    ...
    try {
      ...
      setProducts(prev => prev.map(p => p._id === productId ? { ...p, images: data.images } : p));
      ...
      <div className="admin-page__header">
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          ...
          <button className="btn btn--outline" onClick={() => csvInputRef.current?.click()} disabled={importing}>
            <Upload size={16} /> {importing ? 'Importing...' : 'Import Excel (Replace)'}
          </button>
